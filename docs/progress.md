...
- **In flight:** the README v1 and promo package on branch `docs/readme-promo` (README rewrite, `SETUP_PROMPT.md`, `llms.txt`, GitHub topics set, `tests/policy` scaffolded with one smoke test). Waiting on: the policy tests themselves (a separate agent writes them from the spec: no `EU`/`GDPR`/`AI Act` in README, `docs/**/*.md`, `SETUP_PROMPT.md`, `llms.txt`; no tracker ID in any tracked text file, pattern built without the literal), then PR and merge. The box spike's human-only part (subscription login inside the box, a run billed to the subscription, one request through the host VPN, share of the weekly limit) is with the owner.
...
- **Decisions taken unless the owner objects:** a denied action ends the turn (the runtime re-prompts); the ACP adapter stays for now despite its unused second CLI copy (123 MB); the README carries Lucide icon names in backticks next to the six differentiators (the same names the UI will use), no emoji anywhere.
